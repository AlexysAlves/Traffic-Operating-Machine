...
directionNumberss = {'right': 0, 'down': 1, 'left': 2, 'up': 3}
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self):
        with open('vehiclist.csv', 'r', encoding='UTF8', newline="") as f:
            vehicinf = []
            reader = csv.reader(f)
            global vindex
            vindexloc = 1
        ...
        print(vehicinf)
        self.lane = int(vehicinf[0])
        lane = int(vehicinf[0])
        self.vehicleClass = vehicinf[1]
        self.rgap = int(vehicinf[2])
        self.waiting = False
        self.waited = False
        self.accelerationDist = 100
        self.speed = float(vehicinf[3]) / 10.0
        self.initialSpeed = float(vehicinf[4]) / 10.0
        self.direction_number = int(vehicinf[5])
        self.direction = vehicinf[6]
        direction = self.direction
        self.x = x[direction][lane]
        self.y = y[direction][lane]
        self.crossed = 0
        self.willTurn_r = int(vehicinf[7])
        self.willTurn_l = int(vehicinf[8])
        self.turned = 0
        self.rotateAngle = 0
